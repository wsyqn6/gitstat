...
```bash
gitstat web                   # 扫描当前目录，启动 Web UI
gitstat web D:/work           # 扫描指定目录
gitstat web D:/work --port 8080   # 自定义端口
```
...
- **Ready to go** — Auto-opens browser on web, bilingual (CN/EN)
...
```bash
gitstat web                      # Scan current dir, start Web UI
gitstat web D:/work              # Scan specified directory
gitstat web D:/work --port 8080  # Custom port
```
